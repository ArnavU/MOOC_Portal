...
const PublishCourse = () => {
    ...
    const handelPublish = async () => {
        ...
        console.log("Publishing course form data:", [...formData]);
        try{
            await editCourseDetails(formData);
            navigate("/dashboard/my-courses");
        } catch (error) {
            toast.error("Error while updating changes");
        }

        const category_id= await course.category;
        console.log("category_id",category_id);
        // const addCourseCategory = await addCourseToCategory({categoryId: category_id, courseId: course._id});
    ...
    <div>
        <div className='rounded-md border-[1px] border-richblack-700 bg-richblack-800 p-6'>
            ...
            <label htmlFor="public" className="inline-flex items-center text-lg">
                ...
                <span className="ml-2 text-richblack-400">Send for Review and Approval</span>
            </label>
            </div>
            <div className="ml-auto flex max-w-max items-center gap-x-4">
                <button disabled={false} onClick={goBack} type="button" className="flex cursor-pointer items-center gap-x-2 rounded-md bg-richblack-300 py-[8px] px-[20px] font-semibold text-richblack-900">Back</button>
